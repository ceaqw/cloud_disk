...
<script>
    ...
    export default {
        ...
        methods: {
             ...
             */
            computeMD5(file) {
                //zrx 暂停按钮置为不可用 按钮的状态变化
                this.$nextTick(() => {
                    //检测md5隐藏开始上传按钮，点击会出错
                    $(`.file_${file.id} .uploader-file-actions .uploader-file-resume`).hide()
                    //检测md5完毕，点击暂停，显示开始上传按钮
                    $(`.file_${file.id} .uploader-file-actions .uploader-file-pause`).on("click", function () {
                        $(`.file_${file.id} .uploader-file-actions .uploader-file-resume`).show()
                    });
                    //检测md5完毕，点击开始上传，隐藏自身
                    $(`.file_${file.id} .uploader-file-actions .uploader-file-resume`).on("click", function () {
                        $(`.file_${file.id} .uploader-file-actions .uploader-file-resume`).hide()
                    });
                })
